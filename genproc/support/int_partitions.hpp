...
inline void CommitChanges(Vector<Vector<int>>& dest, Vector<int> insertible,
                          TFilter& checker) {
...
template <Filter TFilter>
inline Vector<Vector<int>> UniquePartitions(PositiveTag, int n,
                                            TFilter instance) {
  if (n == 0) {
    return {{}};
  }
  ...
  while (true) {
    CommitChanges(ans, Vector<int>(numbers.begin(), numbers.begin() + idx + 1),
                  instance);
...
template <Filter TFilter>
inline Vector<Vector<int>> UniquePartitions(NegativeTag, int n,
                                            TFilter instance) {
  ...
  TInvertedFilter filter{.impl = std::move(instance)};
...
template <Filter TFilter>
inline std::vector<std::vector<int>> UniquePartitions(
    int n, TFilter instance = TFilter()) {
  if (n >= 0) {
