...
requires std::is_invocable_r_v<void, Task> &&
    std::is_base_of_v<weave::executors::IExecutor, Executor>
class Batcher {
 public:
  Batcher(Executor& exe, std::size_t batch_size)
      : executor_(exe),
 ...
 private:
  using FutureType = decltype(weave::futures::Submit(std::declval<Executor&>(),
                                                     std::declval<Task>()));
  ...
  void Flush() {
    auto status = weave::futures::no_alloc::All(std::move(batch_)) |
                  weave::futures::ThreadAwait();
...
using BatcherExecutor =
    typename support::detail::BatcherTraits<T>::ExecutorType;
