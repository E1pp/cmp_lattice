...
static std::optional<detail::FunctionFromFile> cached_h0ns;
static std::optional<detail::FunctionFromFile> cached_h0r;

///////////////////////////////////////////////////////////////////////////////

void DefaultHamiltonian::StartSeries(double r_min, double r_max, size_t r_n,
                                     size_t /*lambda*/, std::string path) {
...
std::complex<double> GetMatrixElement(std::vector<int>& state, Sector sector,
                                      double scaling) {
...
DefaultHamiltonian::DefaultHamiltonian(std::vector<std::vector<int>>& ns_states,
...
std::complex<double> DefaultHamiltonian::GetMatrixElement(size_t i, size_t j) {
  ...
  if (i < ns_states_.size()) {
    ...
    WHEELS_VERIFY(i - ns_states_.size() < r_states_.size(),
                  "Index is too big!");

    return tffsa::GetMatrixElement(r_states_[i - ns_states_.size()], Sector::R,
                                   scaling_);
  }
}
