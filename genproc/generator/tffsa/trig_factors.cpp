...
double Tan(double x, double y) {
    return (y - x) / (std::sqrt(1 + std::pow(x, 2.0)) + std::sqrt(1 + std::pow(y, 2.0)));
}

double Cot(double x, double y) {
    return (std::sqrt(1 + std::pow(x, 2.0)) + std::sqrt(1 + std::pow(y, 2.0))) / (x - y);
}
...
double TanFactor(const std::vector<int>& state, Sector sector, double scale_parameter) {
    ...
    for (size_t i = 0; i < state.size(); ++i) {
        for (size_t j = 0; j < i; ++j) {
            product *= Tan(state[i] * sector_multiplier / scale_parameter, state[j] * sector_multiplier / scale_parameter);
        }
    }
