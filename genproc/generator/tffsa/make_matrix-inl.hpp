...
#error "Direct inclusion of this file is not allowed. Include make_matrix.hpp instead"
    ...
    Executor& exe)
{
    ...
    support::Batcher<FillPerturbationMatrixTask, Executor> batcher(exe, 4096 * num_threads);

    for (std::size_t i = 0; i < ns_states.size(); ++i) {
        for (std::size_t j = 0; j < r_states.size(); ++j) {
            batcher.Submit(FillPerturbationMatrixTask(
                perturbation_factor,
                scaling_parameter,
                matrix,
                i,
...
template <Hamiltonian HamiltonianPolicy, typename Executor>
    requires std::is_base_of_v<weave::executors::IExecutor, Executor>
arma::Mat<std::complex<double>> AsyncMakeMatrix(int momentum, int cutoff, double scaling_parameter, double perturbation_factor, Executor& exe)
{
    ...
    using HamiltonianBatcherType = support::Batcher<FillHamiltonianMatrixTask<HamiltonianPolicy>, Executor>;

    MakeHamiltonianMatrix<HamiltonianBatcherType>(
        scaling_parameter,
        ns_states,
        r_states,
        matrix,
...
arma::Mat<std::complex<double>> MakeMatrix(int momentum, int cutoff, double scaling_parameter, double perturbation_factor)
{
    ...
    auto matrix = detail::AsyncMakeMatrix<HamiltonianPolicy>(momentum, cutoff, scaling_parameter, perturbation_factor, pool);
